...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() {
		...
		int numberOfOQuakes = 0;
		for (Marker country : this.countryMarkers) {
			String c = country.getStringProperty("name");
			int numberOfLQuakes = 0;
			for (Marker quakes : this.quakeMarkers) {
				String q = quakes.getStringProperty("country");
				if (quakes instanceof LandQuakeMarker) {
					if (c.compareToIgnoreCase(q) == 0) {
						numberOfLQuakes++;
					}
				}
			}
			if (numberOfLQuakes > 0)
				System.out.printf("Country: %s %d\n",c, numberOfLQuakes);

		}
		for (Marker quakes : this.quakeMarkers) {
			if (quakes instanceof OceanQuakeMarker) {
				numberOfOQuakes++;
			}
		}

		System.out.printf("OCEAN QUAKE %d ", numberOfOQuakes);
	}
